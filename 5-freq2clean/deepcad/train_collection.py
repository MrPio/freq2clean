...
class training_class:
        ...
        for epoch in range(0, self.n_epochs):
            ...
            for iteration, (input, target) in enumerate(trainloader):
                ...
                if iteration % 1 == 0:
                    time_end = time.time()
                    print(
                        "\r[Epoch %d/%d] [Batch %d/%d] [Total loss: %.2f, L1 Loss: %.2f, L2 Loss: %.2f] [ETA: %s] [Time cost: %.2d s] [a0: %.5f, a1: %.5f, a2: %.5f]     "
                        % (
                            epoch + 1,
                            self.n_epochs,
                            iteration + 1,
                            ...
                            time_left,
                            time_end - time_start,
                            self.local_model.module.freq2clean.alphas[0].item(),
                            self.local_model.module.freq2clean.alphas[1].item(),
                            self.local_model.module.freq2clean.alphas[2].item(),
                        ),
                        end=" ",
                    )
    ...
    def save_model(self, epoch, iteration):
        ...
        """
        model_save_name = (
            self.pth_path + "//E_" + str(epoch + 1).zfill(2) + "_Iter_" + str(iteration + 1).zfill(4) + ".pth"
        )
    ...
    def test(self, train_epoch, train_iteration):
        ...
        name_list, noise_img, coordinate_list, test_im_name, img_mean, input_data_type = test_preprocess_chooseOne(
            self, img_id=0
        )
