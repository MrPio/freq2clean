...
def parse_args():
    ...
    parser.add_argument("--dataset", type=Path, default="oabf_astro", help="Testset folder")
...
encoder_checkpoint = args.checkpoint.parent / f"enc_{args.checkpoint.stem}"
...
ds = NoisyDataset(name=args.dataset, augument=False, max_frames=100)
rec = Recording(DATASETS[args.dataset] / "y.tiff", max_frames=300)
frames = rec.frames
rec = rec.np
